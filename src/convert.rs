...
use crate::ast::{
    CaseClause, CaseClauseFlags, Command, ConditionalExpr, ListOp, Redirect, RedirectTarget,
    RedirectType, Word,
};
use crate::ffi;
...
unsafe fn convert_command_with_depth(cmd: *const ffi::COMMAND, depth: usize) -> Option<Command> {
    if depth > MAX_DEPTH {
...
unsafe fn convert_cond_com(cond: *const ffi::COND_COM, depth: usize) -> Option<ConditionalExpr> {
    if depth > MAX_DEPTH {
...
unsafe fn convert_word_list(list: *mut ffi::WORD_LIST) -> Vec<Word> {
...
unsafe fn convert_word_list_to_strings(list: *mut ffi::WORD_LIST) -> Option<Vec<String>> {
    if list.is_null() {
        return None;
    }
...
unsafe fn convert_redirects(redirects: *mut ffi::REDIRECT) -> Vec<Redirect> {
